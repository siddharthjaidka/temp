let tooltipListEl = Array.from(
  document.querySelectorAll('[data-mx-toggle="tooltip"]')
);
let tooltips = [];
//tooltip class
class Tooltip {
  constructor(element, option) {
    ...
    this.placement =
      option.placement || element.getAttribute("data-mx-placement");
  ...
  updatePosition() {
    ...
    let { top, left } = getTooltipPosition(this.element, placement, this.dom);
    this.dom.style.top = top - 5 + "px";
  ...
  switch (placement) {
    ...
    case "bottom":
      top =
        ...
        (targetEl.clientHeight - tooltip.clientHeight);
      left = targetEl.offsetLeft;
      break;
  }
  return {
...
function overflowXCheck() {
  if (document.querySelector("html").scrollWidth > document.body.clientWidth)
    return true;
  else return false;
}
 